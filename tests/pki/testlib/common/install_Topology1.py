...
class PkiInstall(object):
    ...
    def Setup_Config(self):
        ...
        pkiconfig.set(
            "DEFAULT", "pki_security_domain_password", constants.SECURITY_DOMAIN_PASSWORD)
    ...
    def Setup_KRAMaster(self, TempFile='/tmp/ca.cfg'):
        ...
        o = subprocess.Popen(
            shlex.split(args), stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()[1]
        print o
        if isinstance(self.MultiHost, QeHost):
            self.MultiHost.transport.put_file(TempFile, '/tmp/ca.cfg')
    ...
    def Import_Cert(self):
        ...
        args1 = 'pk12util -d %s -i /root/.dogtag/pkitomcat100/test10.p12 -W redhat -k redhat' % (
            self.profile_dir)
